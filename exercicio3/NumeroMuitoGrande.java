...
public class NumeroMuitoGrande {
    ...
    public void multiplicar(int n1, int n2) {
        int cont = 0, flag = 0, i, dect = 0, nn1 = 29 - n1, nn2 = 29 - n2;
        for (int p = 29; p >= 0; p--) {
            mult[p] = 0;
            fica[p] = 0;
        }
        for (int x = 29; x > nn2; x--) {
            ...
            for (int k = 29; k > nn1; k--) {
                ...
                if ((num1[k] * num2[x] + fica[i]) >= 10) {
                    fica[z] = (num1[k] * num2[x] + fica[z]) % 10;
                    flag = 1;
                } else {
                    fica[z] = num1[k] * num2[x] + fica[z];
                }
                if (flag == 1) {
                    flag = 0;
                    int ok = cont - 1;
                    fica[i - ok] = (num1[k] * num2[x] + fica[z]) / 10;
                }
                i--;
            }
            for (int g = 29; g >= 0; g--) {
                if (i == 0) {
                    if ((mult[g] + fica[g]) > 10) {
                        System.out.println("Os numeros adentrados sao maiores dos que os suportados para a multiplicacao!");
                        op1 = 1;
                        break;
                    }
                }
